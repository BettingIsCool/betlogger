...
global placeholder1 = st.empty()
global placeholder2 = st.empty()
global placeholder3 = st.empty()
global placeholder4 = st.empty()
...
display_landing_page(placeholder1, placeholder2, placeholder3, placeholder4)
  ...
  if selected_bookmakers != '()':
    ...
    if selected_tags != '()':
      ...
      if user_unique_starts is not None:
        ...
        styled_df = bets_df.style.applymap(color_cells, subset=['ST', 'P/L', 'EXP_WIN', 'CLV']).format({'ODDS': '{:,.3f}'.format, 'STAKE': '{:,.2f}'.format, 'P/L': '{:,.2f}'.format, 'CLS': '{:,.3f}'.format, 'CLS_TRUE': '{:,.3f}'.format, 'CLS_LIMIT': '{:,.0f}'.format, 'EXP_WIN': '{:,.2f}'.format, 'CLV': '{:,.2%}'.format, 'SH': '{0:g}'.format, 'SA': '{0:g}'.format})
