      ...
      if selected_bookmakers != '()':
        ...
        if selected_tags != '()':
          ...
          if user_unique_starts is not None:
            ...
            styled_df = bets_df.style.applymap(color_cells, subset=['STATUS', 'P/L', 'EXP_WIN', 'CLV']).format({'LINE': '{:g}'.format, 'ODDS': '{:,.3f}'.format, 'STAKE': '{:,.2f}'.format, 'P/L': '{:,.2f}'.format, 'CLS': '{:,.3f}'.format, 'CLS_TRUE': '{:,.3f}'.format, 'CLS_LIMIT': '{0:g}'.format, 'EXP_WIN': '{:,.2f}'.format, 'CLV': '{:,.2%}'.format})
